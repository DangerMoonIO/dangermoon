...
contract DangerMoonBulkBuy is Ownable {
    ...
    uint256 public commissionBips = 75; // 0.75%
    ...
    function dangerMoonBulkBuyBnb(uint256 amountOutMin) payable public {

        require(msg.value > 0, "Not enough BNB sent");

        // pay commissionBips
        uint256 commission = msg.value * commissionBips / 10000;
        payable(owner()).transfer(commission);

        // swap BNB into dangermoon via pcs
        ...
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value:address(this).balance}(
            amountOutMin, // prevents bots frontrunning this contract
            path,
            msg.sender,
            block.timestamp
        );
    ...
    receive() external payable {
        dangerMoonBulkBuyBnb(0);
    }

    function setCommissionBips(uint256 _commissionBips) public onlyOwner {
